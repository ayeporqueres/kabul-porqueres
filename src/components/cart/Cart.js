...
const Cart = () => {
    ...
    const generarPedido = (datosCompraTerminada) => {
        ...
        const itemCollection = dbConection.collection("orders");
    ...
    const actualizarStock = () => {
        ...
        const dbConection =  getFirestore();
        const itemCollection = dbConection.collection("productos");
        itemCollection.get()
            .then(res => {
                res.forEach(item => {
                    cartList.forEach(el => {
                        if (item.data().id === el.id) {
                            const consulta = doc(dbConection, "productos", item.id);
